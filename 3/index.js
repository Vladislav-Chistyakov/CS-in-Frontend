class MyBCD {
  ...
  at(index) {
    if (this.countByte === 0) {
      ...
      if (index < 0) {
        const inversionIndex = this.odd ? ~index + 1 : ~index 
        const oddInex = !!(inversionIndex % 2)
        const indexByte = this.countByte - Math.floor(inversionIndex / 2) - 1
        const byte = this.is8Array[indexByte]

        const firstNumber = byte >> 4
        const lastNumber = byte & 0b00001111

        return oddInex ? firstNumber : lastNumber
      }

      const oddInex = !!(index % 2)

      const indexByte = Math.floor(index / 2)

      const byte = this.is8Array[indexByte]
      const firstNumber = byte >> 4
      const lastNumber = byte & 0b00001111
      return oddInex ? lastNumber : firstNumber
    }
  }
}

const n = new MyBCD(-12345, true);
console.log('BCD CLASS ', n.at(-3))